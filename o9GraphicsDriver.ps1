        ...
        Write-Host "o" -ForegroundColor White -NoNewline; Write-Host "9`n" -ForegroundColor Cyan

        Write-Host "7Z`n"
        ## explorer "https://www.7-zip.org"

# check 7zip installed
$Check7Zip = Test-Path "C:\Program Files\7-Zip\7z.exe"
...
$CheckChrome = (Test-Path "$env:ProgramFiles\Google\Chrome\Application\chrome.exe") -or (Test-Path "${env:ProgramFiles(x86)}\Google\Chrome\Application\chrome.exe")
...
Get-ScheduledTask | Where-Object {$_.Taskname -match 'GoogleUpdaterTaskSystem'} | Unregister-ScheduledTask -Confirm:$false
        ...
        function show-menu {
        Clear-Host
        Write-Host "INSTALL GRAPHICS DRIVERS" -ForegroundColor Cyan
        Write-Host "SELECT YOUR SYSTEM'S GPU`n" -ForegroundColor Cyan
        ...
        Write-Host "DOWNLOAD NVIDIA GPU DRIVER`n" -ForegroundColor Cyan
...
$CheckNvidia = (Test-Path "$env:TEMP\NvidiaDriver.exe") -or (Test-Path "$env:SystemRoot\Temp\NvidiaDriver.exe")
...
Wait-Process -Name chrome
        ...
        Write-Host "SELECT DOWNLOADED DRIVER`n" -ForegroundColor Cyan
...
<ArrayOfProfile>
  <Profile>
    <ProfileName>o9 Profile</ProfileName>
    <Executables/>
    <Settings>
      <ProfileSetting>
...
Start-Process -wait "$env:SystemRoot\Temp\Inspector\nvidiaProfileInspector.exe" -ArgumentList "-silentImport -silent $env:SystemRoot\Temp\Inspector.nip"
        ...
        Write-Host "DOWNLOAD AMD GPU DRIVER`n" -ForegroundColor Cyan
...
cmd /c "reg add `"HKCU\Software\AMD\CN\VirtualSuperResolution`" /v `"AlreadyNotified`" /t REG_DWORD /d `"1`" /f >nul 2>&1"
        ...
        Write-Host "DOWNLOAD INTEL GPU DRIVER`n" -ForegroundColor Cyan
...
cmd /c "reg add `"$regPath`" /v `"Global_LowLatency`" /t REG_DWORD /d `"0`" /f >nul 2>&1"
        ...
        break MainLoop
          ...
          Write-Host "Invalid input. Please select a valid option (1-4).`n" -ForegroundColor Cyan
          Pause
          show-menu
          }
          }
        ...
        Write-Host "- SOUND" -ForegroundColor Cyan
        Write-Host "- RESOLUTION" -ForegroundColor Cyan
        Write-Host "- REFRESH RATE" -ForegroundColor Cyan
        Write-Host "- PRIMARY DISPLAY`n" -ForegroundColor Cyan
		## shell:appsFolder\NVIDIACorp.NVIDIAControlPanel_56jybvy8sckqj!NVIDIACorp.NVIDIAControlPanel
    	## ms-settings:display
		## mmsys.cpl
